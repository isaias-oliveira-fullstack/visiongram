<template>
	<div class="relative sm:max-h-[868.5px] cursor-pointer">
		<template v-if="reel.reelMedia.type === 'video'">
			<video
				:id="reel.id"
				:muted="isVideoMuted"
		...
		<template v-else>
			<img
				:src="reel.reelMedia.mediaUrl"
				:alt="reel.reelMedia.title ?? reel.caption ?? 'Reel image'"
				class="sm:rounded cursor-pointer h-dynamic-screen 
				sm:h-auto max-w-none md:max-w-full md:w-11/12"
			/>
		</template>
...
import UserCard from './SmallCard.vue'

// Video handlers
const isVideoMuted = ref<boolean>(true) // Verifica se o vídeo está atualmente mutado
const isVideoPlaying = ref<boolean>(false) // Verifica se o vídeo está sendo reproduzido
...
/**
* Atualiza o estado de reprodução do vídeo para rastreamento
*/
